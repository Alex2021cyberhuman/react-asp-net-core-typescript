...
public class SpaProxy
{
    ...
    private static readonly HashSet<string> NotForwardedHttpHeaders = new(
        new[] { "Connection" },
    ...
    private static readonly HashSet<string> NotForwardedWebSocketHeaders = new(
        new[]
        {
            "Accept", "Connection", "Host", "User-Agent", "Upgrade", "Sec-WebSocket-Key", "Sec-WebSocket-Protocol",
...
#pragma warning disable CA1068
    public static async Task<bool> PerformProxyRequestAsync(HttpContext context,
#pragma warning restore CA1068
        ...
        bool proxy404)
    {
        ...
        try
        {
            ...
            {
                return await AcceptProxyWebSocketRequest(context, ToWebSocketScheme(targetUri), proxyCancellationToken);
            }
        ...
        catch (HttpRequestException ex)
        {
            throw new HttpRequestException(
                $"Failed to proxy the request to {targetUri}, because the request to " +
                $"the proxy target failed. Check that the proxy target server is running and " +
                $"accepting requests to {baseUri}.\n\n" +
    ...
    private static async Task<bool> AcceptProxyWebSocketRequest(HttpContext context, Uri destinationUri,
        CancellationToken cancellationToken)
    {
        if (context == null)
        {
        ...
        await Task.WhenAll(
            PumpWebSocket(client, server, DefaultWebSocketBufferSize, cancellationToken),
            PumpWebSocket(server, client, DefaultWebSocketBufferSize, cancellationToken));
